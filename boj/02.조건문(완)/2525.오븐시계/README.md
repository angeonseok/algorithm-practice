...
### 핵심 아이디어
요리 시간을 시/분으로 쪼개서 더하고 넘치는 만큼 올려줬다. c // 60으로 시간부터 밀고 c % 60만 분에 붙이는 식.

### 시간복잡도
O(1). 나눗셈 두 번에 보정 두 번.

### 구현 포인트
- c가 60 넘게 들어오니까 시간부터 c // 60 만큼 올려주고 나머지 분만 더한다
- 그러고도 분이 60 넘을 수 있는데 최대 한 번만 넘으니 if 하나로 충분
- 24시 넘어가면 -24 한 번이면 ㅇㅋ. while 안 돌려도 됨
