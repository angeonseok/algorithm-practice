...
### 핵심 아이디어
i~j 구간만 잘라내서 reverse 하고 다시 그 자리에 꽂는다. 슬라이스 대입이 제일 짧다.

### 시간복잡도
O(N * M). 뒤집기 한 번이 최대 N이고 그걸 M번.

### 구현 포인트
- k[i:j+1] 로 잘라야 j번째까지 포함됨. 슬라이싱 범위 조심
- 여기도 0번바구니는 없다. n+1 배열 만들고 1..n에 자기 번호 채워둠
- temp.reverse()는 제자리 뒤집기라 반환값이 없다. temp = temp.reverse() 하면 None 박힘
