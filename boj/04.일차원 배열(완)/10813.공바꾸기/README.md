...
### 핵심 아이디어
두 칸 값을 맞바꾸는 것만 M번. 파이썬 다중 대입으로 한 줄에 스왑.

### 시간복잡도
O(N + M). 교환 한 번은 상수라 명령 수만큼이고, 배열 만들고 출력하는 데 N.

### 구현 포인트
- 여긴 앞의 두 문제랑 다르게 list(range(1, n+1))로 0-인덱스 배열을 씀. 그래서 접근할 때 i-1, j-1. 범위 조심
- 임시변수 없이 k[i-1], k[j-1] = k[j-1], k[i-1] 한 줄이면 됨
- 출력은 0번부터 전부 돌면 됨. 앞 문제들처럼 1부터 시작하면 첫 칸이 날아감
