...
### 핵심 아이디어
실제 말 개수 [1, 1, 2, 2, 2, 8] 박아놓고 입력받은 거랑 자리별로 뺀다. 걍 리스트 2개 만들어서 빼면 되는거 아님?

### 시간복잡도
O(1). 6번 빼고 나가면 됨.

### 구현 포인트
- b 리스트 순서가 킹 퀸 룩 비숍 나이트 폰. 순서 어긋나면 그대로 틀림
- 모자라면 양수 남으면 음수로 나가야 하니까 절댓값 씌우면 안 됨
- 답 6개가 한 줄에 나가야 한다. end=' ' 빼먹으면 세로로 줄줄이 나감
