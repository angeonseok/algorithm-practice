...
### 핵심 아이디어
그리디. 25, 10, 5, 1 큰 단위부터 몫 세고 나머지를 다음 단위로 넘기면 동전 개수가 최소가 됨.

### 시간복잡도
O(T * 4). 케이스마다 동전 4종류만 훑고 끝나서 사실상 상수.

### 구현 포인트
- m = [25, 10, 5, 1] 이 순서가 곧 출력 순서라 나중에 정렬할 게 없음
- i // j 로 개수 담고 i %= j 로 금액 깎아가는 게 전부
- 답은 ans에 다 몰아 담았다가 print(*ans)로 언패킹해서 한 번에 뱉음
