...
### 핵심 아이디어
한 변에 놓인 점 개수가 2 > 3 > 5 > 9 로 가는 게 보인다. N번 거치면 한 변이 2^N + 1개고 격자니까 제곱하면 끝.

### 시간복잡도
O(1). 거듭제곱 한 방.

### 구현 포인트
- n=1일 때 3^2, n=2일 때 5^2 손으로 적다 보면 (2^n + 1)^2 꼴이 나온다
- 중복 점을 한 번만 저장한다는 조건이 결국 격자 점 개수 세라는 소리
- N이 15까지라 파이썬 정수면 크기 걱정 없음
