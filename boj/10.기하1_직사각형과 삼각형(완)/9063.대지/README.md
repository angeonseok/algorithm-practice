...
### 핵심 아이디어
점 전부 감싸는 최소 직사각형이니까 바운딩 박스. x축 길이는 최대x - 최소x, y축도 똑같이 구해서 곱하면 넓이다.

### 시간복잡도
O(N). N개 점 입력받고 max/min으로 한 번씩 훑는 게 끝. N이 10만이라 여유롭다.

### 구현 포인트
- 안쪽 점들은 아무 의미 없음. 필요한 건 x 최대/최소, y 최대/최소 네 개뿐
- 좌표가 음수로도 들어오는데 뺄셈이라 알아서 처리됨
- 같은 자리에 여러 개 있거나 한 줄로 서 있으면 넓이 0. max-min이 그냥 0을 뱉어서 예외처리 필요없다
