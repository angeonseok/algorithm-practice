...
### 핵심 아이디어
간선을 순서대로 하나씩 넣다가 두 끝점 루트가 이미 같으면 그 턴에 사이클이 완성된 거다. 끝까지 안 걸리면 0.

### 시간복잡도
O(M α(N)). 간선 M개를 순서대로 딱 한 번씩만 본다.

### 구현 포인트
- find 재귀로 했다가 터짐. 루트 찾는 while + 경로 압축 while 두 번 도는 반복문 버전으로 갈아엎음
- 정점 번호가 0부터라 parent = list(range(n)). n+1 아니다
- for-else 구조라 break 안 걸리고 끝까지 돌면 0 출력
