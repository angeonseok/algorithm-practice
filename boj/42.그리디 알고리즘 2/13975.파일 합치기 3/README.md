...
### 핵심 아이디어
매번 제일 작은 두 놈을 합치는 그리디. 허프만이랑 같은 얘기고, 최소 2개를 계속 뽑아야 하니 힙이 딱이다.

### 시간복잡도
O(T * N log N). T는 테스트케이스 수, N은 파일 개수. heapify 하고 N-1번 합치는데 pop/push가 log N.

### 구현 포인트
- 합친 결과 c를 다시 힙에 넣어야 한다. 걔도 다음 합치기 후보임
- 정답은 최종 합계가 아니라 매번 합칠 때 든 비용 c를 전부 누적한 값
- 리스트 정렬해두고 앞 두 개 빼는 식으로 하면 다시 넣을 자리 찾느라 터진다. heapify 한 방이 편함
- T가 있는 문제라 테케마다 힙 새로 만드는 거 잊지 말자
