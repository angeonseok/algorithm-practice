...
### 핵심 아이디어
간선 삭제를 그대로 따라가면 답이 없다. 질문을 역순으로 처리하면 삭제가 연결로 바뀌니까 처음엔 전부 끊긴 상태로 두고 유니온 파인드로 붙여나가면 됨. 쿼리 다 받아놓고 뒤에서부터 훑는 오프라인 처리.

### 시간복잡도
O((N + Q) * α(N)). N은 정점 수, Q는 질문 수. 쿼리 전체를 한 번만 역방향으로 훑는다.

### 구현 포인트
- 재귀 find는 43점, 반복문으로 바꾸니 100점 나왔다. n이 20만이라 재귀 깊이가 감당이 안 됨
- parent 배열이 두 개다. 유니온 파인드용 parent랑 문제에서 준 tree_parent 헷갈리지 말 것
- 쿼리를 n-1+q 개 전부 받아둬야 역순 처리가 성립한다
- 역순으로 모은 답이니까 출력할 때 다시 reversed 걸어서 돌려놔야지
