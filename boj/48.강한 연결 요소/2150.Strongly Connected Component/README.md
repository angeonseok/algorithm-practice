...
### 핵심 아이디어
코사라주로 간다. 정방향 dfs로 종료 순서를 쌓아놓고, 그 역순으로 역방향 그래프에서 dfs 돌리면 한 번에 묶이는 덩어리가 곧 SCC.

### 시간복잡도
O(V + E). dfs를 두 번 돌리는 거라 상수만 2배.

### 구현 포인트
- 재귀를 2번 돌려서 setrecursionlimit 걸어야 함
- dfs1은 다 보고 나오면서 order에 append. 이 종료 순서를 뒤에서부터 꺼내 쓰는 게 전부다
- 정방향, 역방향 그래프 둘 다 만들어야 됨. 입력 받을 때 graph[a].append(b), r_graph[b].append(a)로 같이 채움
- 출력 조건이 빡세다. SCC 안은 오름차순, SCC끼리는 최소 정점 기준 정렬, 줄 끝엔 -1
