...
### 핵심 아이디어
*을 뺀 나머지 가중합을 먼저 구해놓고 0부터 9까지 대입해보면서 10으로 나눠떨어지는 놈을 찾았다. 후보가 10개밖에 없어서 식 정리하는 것보다 이게 편하다.

### 시간복잡도
O(1). 13자리 고정 + 후보 10개라 상수.

### 구현 포인트
- 가중치는 인덱스 짝수면 1배, 홀수면 3배. 0-based라 헷갈리니 인덱스 조심
- *이 몇 번째 자리인지 index로 먼저 잡아둬야 후보를 1배로 넣을지 3배로 넣을지 정함
- *은 continue로 건너뛰고 합에서 빼놓는 게 포인트
