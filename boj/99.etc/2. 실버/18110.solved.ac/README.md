...
### 핵심 아이디어
정렬하고 위아래 15%씩 잘라낸 뒤 평균. 반올림이 두 번 나오는데 파이썬 기본 반올림이 은근 안 맞아서 0.5 더하고 int로 내리는 방식으로 처리했다.

### 시간복잡도
O(N log N). 정렬이 제일 비싸고 나머진 슬라이스 한 번에 합 한 번.

### 구현 포인트
- 파이썬 올림을 믿지말자. round는 짝수 쪽으로 붙어서 0.5 더하고 int() 하는 게 안전하다
- n == 0이면 의견이 없는 거라 그냥 0 뱉고 끝내야지. 안 그러면 0으로 나눠서 터짐
- 자르는 개수 cut은 위아래 동일하게 arr[cut:n-cut] 로 한 방에 슬라이스
