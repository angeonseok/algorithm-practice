...
### 핵심 아이디어
PD가 준 순서를 인접한 쌍끼리만 간선으로 쪼개고 위상정렬. 진입차수 0인 놈부터 큐에 넣고 빼면서 순서 뽑음.

### 시간복잡도
O(V + E). 간선이 입력 목록 길이만큼만 생겨서 입력 크기 그대로 훑는 수준.

### 구현 포인트
- 입력 첫 숫자가 가수 수라 s[0] 떼고 s[1:]만 씀. 들어오는 입력형태를 잘 보자
- k개 목록이면 간선은 k-1개. arr[i] > arr[i+1]로만 이으면 되고 전부 연결시킬 이유 없음
- 뽑은 결과 길이가 n이 아니면 사이클 낀 거라 0 출력
