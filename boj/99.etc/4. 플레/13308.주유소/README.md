...
### 핵심 아이디어
다익스트라에서 dp냄새가 난다. 상태에 지금까지 본 최저 기름값을 같이 달고 다니면서 (도시, 기름값)로 거리를 따로 관리했다.

### 시간복잡도
O(C * M log(N * C)). C가 기름값 최대치라 (도시, 기름값) 상태가 N*C개 생기고, 간선도 기름값 종류만큼 다시 훑게 됨.

### 구현 포인트
- dist를 [도시][기름값] 2차원으로 잡음. 처음엔 defaultdict 썼는데 못알아먹겠어서 갈아엎음
- 비용은 지금 기름값 * 간선 길이로 더하고, 도착한 도시에서 min으로 기름값 갱신
- u == n 만나면 바로 break. 이거 넣으니까 100점 나옴
- 기름값 인덱스를 도시 번호랑 맞추려고 앞에 더미 0 하나 박음
