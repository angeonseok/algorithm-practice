...
### 핵심 아이디어
명령 그대로 시뮬 돌린다. 1이면 속도 올리고 2면 내리고 0이면 유지, 그리고 매 초 끝에 현재 속도만큼 거리를 더함.

### 시간복잡도
O(T * N). 명령 개수만큼 한 번씩 처리하는 게 다.

### 구현 포인트
- 감속해서 속도가 음수로 내려가면 0으로 잘라야 함. 뒤로 가는 건 없다
- dist += speed 는 if 블록 밖에. 명령이 0이어도 그 초에 이동은 하잖
- 입력 토큰 수가 명령마다 다르다. 0은 한 개, 1/2는 두 개. C++은 cmd 먼저 읽고 필요할 때만 acc를 더 읽고, 파이썬은 줄을 통째로 리스트로 받아서 cmd[1] 있을 때만 씀
