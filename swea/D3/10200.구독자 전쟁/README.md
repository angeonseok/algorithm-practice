...
### 핵심 아이디어
벤다이어그램 그려보면 바로 견적 나온다. 겹치는 최대는 min(a, b), 최소는 a+b가 n을 넘치는 만큼.

### 시간복잡도
O(T). 테케당 min/max 두 번이 끝.

### 구현 포인트
- 최솟값은 max(0, a+b-n). 다 합쳐도 n을 안 넘으면 아예 안 겹칠 수 있으니 0
- 출력이 최댓값 먼저, 최솟값 나중이다. 순서 뒤집으면 그냥 틀림
- 배열 만들 일도 없고 사람 하나하나 볼 일도 없음
