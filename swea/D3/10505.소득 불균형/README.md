...
### 핵심 아이디어
평균 한 번 구하고 그 이하인 놈 개수만 세면 된다. 대신 평균은 정수로 버려서 쓴다.

### 시간복잡도
O(T * N). 합 구할 때 한 번, 셀 때 한 번 훑는다.

### 구현 포인트
- cpp는 avg /= n 으로 int 나눗셈, 파이썬은 int(sum/n) 으로 똑같이 버림 처리
- 이하니까 <= 로 비교. < 로 두면 평균과 딱 같은 놈이 빠짐
- 나눗셈이 문제다 항상. 실수 평균 그대로 쓰면 경계에서 어긋난다
