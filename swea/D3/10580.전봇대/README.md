...
### 핵심 아이디어
왼쪽 고도로 정렬해두면 교차 = 오른쪽 고도 순서가 뒤집힌 쌍이 된다. 결국 역전 개수 세기.

### 시간복잡도
O(T * N^2). N이 1000이라 이중 for로 50만 번 비교, 이 정도면 ㄱㅊ.

### 구현 포인트
- sort는 a 기준으로 먹는다. 끝점이 겹치는 경우가 없다니까 동점 걱정도 없음
- 교차 조건은 lst[i][1] > lst[j][1] 딱 하나
- N이 더 컸으면 이중 for는 터지고 머지소트나 BIT로 역전 세러 가야 함
