...
### 핵심 아이디어
15판 중 8승이 목표니까 남은 판은 전부 이긴다고 치면 됨. 이미 진 판 개수만 세서 8 넘었는지 보면 끝.

### 시간복잡도
O(T * |S|). 문자열이 최대 15자라 세는 것도 상수 취급.

### 구현 포인트
- 'o'가 아니라 'x'를 센다. 진 게 8개면 남은 거 다 이겨도 8승이 안 나옴
- 남은 경기 수를 따로 안 구해도 됨. 가능성만 보는 거라 낙관적으로 가정
- cnt < 8 경계 조심. 딱 8이면 이미 NO
