...
### 핵심 아이디어
보이는 괄호는 전부 공의 흔적인데, '()'로 딱 붙어 있는 놈만 두 글자가 공 하나임. 괄호 총 개수에서 붙어 있는 쌍 개수를 빼면 최솟값.

### 시간복잡도
O(T * |S|). 길이 50짜리 문자열 두 번 훑는 정도.

### 구현 포인트
- 잡초에 완전히 가려진 공은 셀 필요 없다. 최솟값이라 안 보이면 없다고 치면 됨
- 짝 안 맞는 외톨이 '(' 나 ')' 도 공 하나로 쳐야지. 반대쪽은 잡초에 먹힌 거니까
- count("()") 는 겹치지 않게 세는데 공끼리 안 겹친다는 조건이 있어서 문제 없음
