...
### 핵심 아이디어
커맨드 한 글자씩 그대로 따라가는 시뮬레이션. 방향 벡터 딕셔너리를 포탄용(dirs)이랑 커맨드용(move) 둘로 나눠 깔아두고 이동/발사만 분기시킴.

### 시간복잡도
O(T * (H*W + N*max(H,W))). 전차 찾느라 격자 한 번 훑고, 커맨드 하나당 최악이 발사라 한 줄 끝까지 직진.

### 구현 포인트
- move 딕셔너리가 (di, dj, 방향문자)까지 들고 있어서 방향 바꾸는 것도 한 번에 처리됨
- 이동이 막혀도 방향은 바뀐다. field[si][sj] = d 먼저 박아놓고 그 다음에 이동 판정
- 포탄은 '*' 만나면 '.'로 바꾸고 소멸, '#'는 그냥 소멸, 맵 밖으로 나가면 아무 일 없음
- 출력 양식 맞추기. '#tc' 뒤 end=" " 로 띄우고 첫 줄이 바로 붙게 join으로 뽑음
