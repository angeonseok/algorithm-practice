...
### 핵심 아이디어
사라진 놈은 남은 여섯보다 커야 하니까 max+1부터 시작. 거기서 1씩 올리면서 총합이 7로 나눠떨어지는 첫 값을 잡는다.

### 시간복잡도
O(T). 후보를 1씩 올려도 나머지가 7 안에서 한 바퀴 다 도니까 while은 길어야 7번.

### 구현 포인트
- 시작값이 max(arr) + 1. 같으면 안 된다는 조건이라 +1 안 하면 박음
- 평균이 정수 = (남은 합 + 후보) % 7 == 0. 나눗셈 말고 나머지로 보면 됨
- 나머지 계산으로 한 방에 점프시킬 수도 있는데 max보다 커야 한다는 조건까지 같이 봐야 돼서 걍 while로 뺑이 돌림
