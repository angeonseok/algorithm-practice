...
### 핵심 아이디어
색이 6개뿐이라 알고리즘 짤 게 없다. 인접 관계 표랑 마주보는 관계 표를 딕셔너리로 박아놓고 조회만 함.

### 시간복잡도
O(T). 표는 상수로 박혀 있고 케이스마다 색 두 개 읽어서 분기만 탐.

### 구현 포인트
- 같은 색(E) 판정을 제일 먼저. 순서 밀리면 엉뚱한 분기로 샘
- table_A는 값이 리스트라 in 으로 보고, table_C는 1:1이라 == 로 봄
- 반시계로 red-orange-yellow-green-blue-purple 순서 적어두고 표 만들면 안 헷갈림
