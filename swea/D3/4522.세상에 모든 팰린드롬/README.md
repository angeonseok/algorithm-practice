...
### 핵심 아이디어
실제 단어를 만들어볼 필요가 없다. 마주보는 두 글자만 보면 됨. 한쪽이라도 물음표면 맞춰줄 수 있고, 둘 다 글자인데 서로 다르면 그 순간 컷.

### 시간복잡도
O(T * L). L은 패턴 길이 20 이하. 절반만 훑으면 판정 끝.

### 구현 포인트
- 가운데 글자는 자기 자신이랑 짝이라 항상 ㅇㅋ. len // 2 까지만 돌리면 됨
- for-else 구문 씀. 중간에 break 안 걸리고 끝까지 돌면 else로 빠져서 Exist 찍힘
- break 하기 전에 Not exist를 먼저 찍어야 출력이 한 줄로 끝남
