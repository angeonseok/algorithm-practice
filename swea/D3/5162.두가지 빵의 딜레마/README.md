...
### 핵심 아이디어
개수만 많으면 장땡이라 싼 빵만 계속 사면 된다. 두 종류 섞을 이유가 하나도 없음.

### 시간복잡도
O(T). 계산이 한 줄이라 더 줄일 것도 없음.

### 구현 포인트
- min(a, b)로 싼 놈 잡고 c 나누면 끝. 조합 돌릴 필요 없다
- 남는 돈은 어차피 버리는 거라 // 만 쓰면 됨
- a랑 b가 같아도 min이 알아서 하나 집어주니까 분기 안 나눠도 됨
