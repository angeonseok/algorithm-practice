...
### 핵심 아이디어
다섯 줄 길이가 제각각인 게 문제라, 제일 긴 줄에 맞춰 빈 칸으로 채워놓고 열 방향으로 훑었다. 없는 칸은 건너뛰기.

### 시간복잡도
O(T * 5L). L은 제일 긴 문자열 길이 15 이하. 패딩 만들고 열 순회 한 번.

### 구현 포인트
- 빈 문자열로 패딩해놔서 if not arr[j][i] 하나로 없는 칸이 걸러짐
- 바깥이 열 i, 안쪽이 행 j 순서. 뒤집어서 돌리면 그냥 가로로 읽어버린다
- list(input())로 받아 인덱스 접근하는 구조라 길이 안 맞춰두면 IndexError 난다
