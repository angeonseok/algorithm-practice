...
### 핵심 아이디어
게임이론처럼 생겼는데 답은 그냥 전부 1/n. 앞사람이 욕심내서 더 따르면 뒷사람이 그 잔을 집어가니까 결국 다 똑같이 1/n씩 따르게 됨.

### 시간복잡도
O(T * N). 계산이랄 게 없고 n번 출력하는 게 전부다.

### 구현 포인트
- 문자열 "1/" + str(n) 하나 만들어서 n번 뱉으면 끝. 분수 계산 자체가 없다
- 1/n은 이미 기약분수라 최대공약수로 나눌 일이 없다
- print에 end=" " 주고 마지막에 빈 print로 줄바꿈. 출력 양식 맞추기
