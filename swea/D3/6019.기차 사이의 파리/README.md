...
### 핵심 아이디어
파리 왕복을 급수로 세면 안 되고, 기차 둘이 만나는 시간 d/(a+b)만 구하면 된다. 그 시간 내내 파리는 f 속력으로 날아다니니까 곱하면 답.

### 시간복잡도
O(T). 식 한 줄.

### 구현 포인트
- 파리가 몇 번 반사되는지는 볼 필요 없다. 충돌 시간 * 파리 속력
- d / (a + b) 는 실수 나눗셈으로 해야 함. 나눗셈이 문제다 항상
- 절대/상대오차 10^-6 허용이라 float 그대로 던져도 통과
