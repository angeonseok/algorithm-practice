...
### 핵심 아이디어
주변 지뢰 0인 칸을 시작점으로 BFS 돌려서 연쇄로 열리는 덩어리를 클릭 1번으로 먹는다. 그러고도 안 열린 안전칸은 하나씩 다 눌러야 하니 따로 셈.

### 시간복잡도
O(T * N^2). 칸마다 8방향 보는 게 상수고 N이 300이라 여유롭다.

### 구현 포인트
- 큐에서 꺼낼 때 count_boom 다시 재서 0이 아니면 continue. 숫자 칸은 표시만 되고 거기서 더 안 퍼진다
- 0짜리 덩어리 전부 처리한 다음에 남은 '.' 칸을 개별 클릭으로 세야 함. 순서 뒤집으면 답이 커진다
- 8방향 dirs 튜플로 박아두고 count_boom을 시작 판정이랑 확장 판정에 재활용
