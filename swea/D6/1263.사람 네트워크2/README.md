...
### 핵심 아이디어
모든 정점 도달 거리의 합이 필요하고 자료도 인접행렬로 준다 > 플로이드 씀. 다 돌리고 나서 행 합이 제일 작은 놈 출력.

### 시간복잡도
O(T * N^3). N이 1000이면 빡세서 안쪽 루프를 최대한 깎아야 한다.

### 구현 포인트
- 입력이 한 줄에 n + 인접행렬 전부라 tmp[0]가 n, 나머지를 n칸씩 잘라서 행렬로 만듦
- 간선 없는 0을 INF로 바꿔야 하는데 i != j 조건 빼먹으면 자기 자신까지 INF 된다
- dist[i][k]가 INF면 j 루프 통째로 continue, 아니면 ik로 빼둔다. 이거 안 하면 느려서 터짐
- CC 최솟값이 답이라 min(sum(row))
