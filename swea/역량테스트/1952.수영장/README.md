...
### 핵심 아이디어
12개월짜리 dp로 간다. i번째 달까지의 최소 비용은 앞 달 결과에 이번 달 선택만 붙이는 구조라 앞만 보면 됨. 1일권/1달권 중 싼 놈 붙이고, 3달권은 dp[i-3]에서 점프시킴.

### 시간복잡도
O(T * 12). 달이 12개로 고정이라 사실상 상수. 테케 수만큼 반복하는 게 전부.

### 구현 포인트
- 1일권은 그 달 이용일수 * 가격이라 days_cost로 미리 뽑아두고 1달권이랑 min 때림
- 3달권은 i >= 3일 때만 본다. 3개월 통째로 덮는 거라 dp[i-1]이 아니라 dp[i-3]에서 온다
- 연간권은 dp에 안 넣고 마지막에 dp[12]랑 한 번만 비교. 어차피 1월 1일 시작 고정이라 중간에 낄 자리가 없다
