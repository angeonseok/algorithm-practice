...
### 핵심 아이디어
즐거운 bfs인데 터널 타입마다 뚫린 방향이 달라서 타입별 방향 후보를 딕셔너리로 박아뒀다. 나만 뚫려있으면 소용없고 상대 칸도 나를 향해 뚫려있어야 이동 가능.

### 시간복잡도
O(T * N * M). 칸마다 한 번씩만 방문하고 방향은 최대 4개라 상수배.

### 구현 포인트
- link()가 핵심. 상대 칸의 방향 목록을 펼쳐서 그 안에 내 좌표가 있는지 확인해야 상호 연결이 성립함
- visited에 방문 여부 대신 경과 시간을 저장해뒀다. visited[x][y] == l이면 더 뻗을 이유 없으니 continue
- 시작 칸도 1로 세고 들어감. 맨홀 자리부터 이미 후보라서 cnt 초기값에 포함시켜야 됨
