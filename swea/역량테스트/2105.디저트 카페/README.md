...
### 핵심 아이디어
대각선 사각형은 방향이 0 > 1 > 2 > 3 순서로만 꺾인다. 그래서 dfs에서 다음 방향 후보를 현재 방향이랑 +1 두 개로만 제한해놓고 완탐 돌렸다.

### 시간복잡도
O(T * N^4). 시작점이 N^2개고 시작점마다 만들 수 있는 사각형이 대략 N^2개.

### 구현 포인트
- visited를 좌표가 아니라 디저트 종류(1~100) 기준으로 잡음. 같은 디저트 두 번 먹으면 안 되니까
- 시작점은 i가 0 ~ n-3, j가 1 ~ n-2까지만 본다. 여기서 벗어나면 사각형 자체가 안 나오니까 미리 컷
- 시작 좌표로 돌아왔을 때 cnt >= 4여야 인정. 한 칸짜리나 왕복은 사각형이 아니다
- nd가 3 넘어가면 continue. 네 번 꺾었으면 이미 한 바퀴 돈 거니까 컷
