...
### 핵심 아이디어
일꾼 두 명을 동시에 완탐하면 견적이 안 나온다. 먼저 모든 (행, 시작열) 구간의 최대 이익을 arr_profit에 다 깔아두고, 그 다음엔 구간 두 개 조합만 골라서 합치는 식으로 나눴다.

### 시간복잡도
O(T * (N^2 * 2^M + N^4)). 구간 하나당 부분집합이 2^M(M<=5)이고 조합 비교가 N^2 * N^2인데 N이 10이라 여유.

### 구현 포인트
- profit()은 구간 안에서 벌통을 넣냐 마냐 부분집합 완탐. 합이 C 넘어가는 순간 바로 return
- 수익이 제곱합이라 total(그냥 합)이랑 cur_sum(제곱 합)을 따로 들고 다녀야 함
- 겹침 판정은 i1 == i2 and abs(j1 - j2) < m 일 때만 컷. 행이 다르면 겹칠 일이 없다
- arr_profit 열 크기가 n이 아니라 n-m+1. 인덱스 조심
