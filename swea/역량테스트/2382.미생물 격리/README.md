...
### 핵심 아이디어
N*N 배열 안 만들고 군집 리스트만 들고 시뮬레이션 돌렸다. 이동 > 최외곽이면 반갈 + 방향 반전 > 같은 칸에 모인 놈들 딕셔너리로 합치기, 이 순서로 M번 반복.

### 시간복잡도
O(T * M * K). M초 동안 매번 K개 군집을 훑는 구조. 1000 * 1000이면 충분히 돌아감.

### 구현 포인트
- pos 딕셔너리 값에 [r, c, 수, 방향, 최대군집수] 로 다섯 번째 칸을 따로 붙임. 합칠 때 방향은 제일 많은 놈 걸 따라가야 해서 max 추적용이 필요
- 미생물 수가 0이 된 놈은 pos에 넣기 전에 걸러버림. 안 그러면 빈 군집이 계속 돌아다닌다
- 최외곽 판정은 이동 후 좌표로. 0이나 n-1에 닿았는지 보고 절반 버림 + 방향 반대로
- 격자를 통째로 관리하는 건 손해다. K가 1000이라 리스트만 굴리는 게 훨씬 싸다
