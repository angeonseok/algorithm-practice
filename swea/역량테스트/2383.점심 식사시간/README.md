...
### 핵심 아이디어
사람마다 1번 계단이냐 2번 계단이냐만 정하는 2^P 완탐. 배정이 끝나면 계단별 종료 시간은 도착 순 정렬 후 3명 제한만 지키면 알아서 최적이 나온다.

### 시간복잡도
O(T * 2^P * P log P). 사람이 최대 10명이라 1024가지 배정에 정렬이 붙는 정도.

### 구현 포인트
- calc()에서 4번째 사람부터 start = max(도착시간, end[i-3]). 계단에 3명뿐이라 3칸 앞 놈이 다 내려가야 들어갈 수 있음
- 계단 입구까지 거리에 +1 해야 됨. 입구 찍고 1분 뒤에 내려가기 시작이라 이거 빼먹으면 전부 틀어진다
- end가 정렬된 도착 순으로 쌓이니까 end[i-3]이 곧 먼저 들어간 3번째 놈의 종료 시각이 됨
- 정답은 두 계단 종료 시간 중 큰 놈이고, 그 큰 놈들 중에서 최소를 고르는 거다
