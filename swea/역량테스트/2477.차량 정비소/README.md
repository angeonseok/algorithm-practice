...
### 핵심 아이디어
우선순위 규칙이 너무 빡세서 딴 수가 안 보인다. 1초씩 흘리면서 접수 창구랑 정비 창구 상태를 직접 굴리는 정직한 시뮬레이션으로 감.

### 시간복잡도
O(T * time * (N + M + K)). 도착 시간이 최대 1000이고 창구 처리 시간도 20 이하라 while이 도는 횟수가 뻔하다.

### 구현 포인트
- 한 틱 안의 처리 순서가 생명. 정비 끝난 놈 정리 > 접수 끝난 놈 정비 대기열로 > 정비 배정 > 접수 배정 > 남은시간 1 감소
- 정비 대기열은 (접수 끝난 시각, 접수 창구 번호) 로 정렬해야 문제 우선순위랑 맞음
- 창구 상태를 [고객번호, 남은시간]으로 들고, 고객번호 != 0 이면서 남은시간 == 0인 순간에만 완료 처리
- used_rec / used_rep에 고객별 창구 번호 남겨뒀다가 마지막에 A, B 둘 다 맞는 번호만 합산. 하나도 없으면 -1
