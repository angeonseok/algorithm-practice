...
### 핵심 아이디어
명령마다 회전 방향을 rot 배열에 먼저 다 기록해두고 마지막에 한 번에 돌린다. 돌리면서 전파를 판정하면 이미 바뀐 값으로 비교하게 돼서 안 됨.

### 시간복잡도
O(T * K). 명령 하나당 톱니 4개만 보고 회전도 리스트 슬라이싱이라 상수 취급.

### 구현 포인트
- 접점은 왼쪽 톱니의 2번 인덱스랑 오른쪽 톱니의 6번 인덱스. 다르면 반대 방향으로 전파, 같으면 break로 컷
- 전파는 num 기준 왼쪽 방향, 오른쪽 방향 따로 훑는다. 중간에 한 번 막히면 그 뒤로는 아예 안 돈다
- 회전은 시계면 [마지막] + 나머지, 반시계면 나머지 + [처음] 슬라이싱으로 처리
- 점수는 12시 방향(0번 인덱스)이 1일 때 톱니 순서대로 1, 2, 4, 8 더하기
