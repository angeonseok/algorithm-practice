...
### 핵심 아이디어
행이든 열이든 결국 1차원 배열 하나 검사하는 문제라 열은 따로 뽑아서 같은 함수에 넣었다. 오르막이면 뒤쪽 X칸, 내리막이면 앞쪽 X칸이 평평한지만 보면 됨.

### 시간복잡도
O(T * N^2 * X). 행 N개 + 열 N개를 각각 길이 N만큼 훑고 경사로 검사가 X칸씩.

### 구현 포인트
- used 배열로 경사로 겹침 체크. 같은 칸에 두 개 못 까니까 이거 없으면 박는다
- 오르막(arr[i]+1 == arr[i+1])은 i에서 뒤로 X칸, 내리막은 i+1부터 앞으로 X칸. 방향 헷갈리기 딱 좋다
- 내리막 처리하면 i += X로 점프시키고 오르막은 i += 1. 내리막은 이미 앞쪽을 다 확인해버린 상태라 그럼
- 높이 차가 2 이상 나면 바로 False. 경사로 높이가 1이라 답이 안 나옴
