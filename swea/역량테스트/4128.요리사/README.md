...
### 핵심 아이디어
재료 N개를 N/2개씩 나누는 조합을 전부 만들어보고 시너지 합 차이 최소를 고른다. N이 16이라 조합이 12870개밖에 안 나와서 뇌 빼고 완탐해도 된다.

### 시간복잡도
O(T * C(N, N/2) * N^2). N=16이면 12870 * 64 수준이라 여유롭게 통과.

### 구현 포인트
- 시너지가 Sij랑 Sji 둘 다 붙는 거라 synergy[a][b] + synergy[b][a] 양방향 다 챙겨야 함
- f2는 f1에 없는 놈들만 골라서 만든다. i not in f1 이 튜플 선형 탐색이라 좀 느린데 n이 작아서 그냥 넘어감
- f1 정하면 f2는 자동 결정. 대칭이라 절반만 봐도 되긴 하는데 걍 다 돌렸다
- 차이의 절댓값 최소를 찾는 거라 ans 초기값은 크게 박아둠
