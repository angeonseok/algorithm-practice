...
### 핵심 아이디어
1초 단위로만 움직이면 서로 스쳐 지나가는 0.5초 충돌을 놓친다. 그래서 입력 좌표를 통째로 2배 뻥튀기해놓고 1칸씩 이동시킴. 반칸에서 만나는 놈들도 정수 좌표로 잡힌다.

### 시간복잡도
O(T * 4000 * N). N은 1000이고 좌표 범위가 -2000~2000이라 4000턴이면 남은 놈 전부 밖으로 나감.

### 구현 포인트
- 매 턴 dict에 좌표별 개수 세고, 2개 이상 모인 자리 원자는 에너지 더하고 버림
- 범위 밖으로 나간 놈은 다시 안 돌아오니까 그냥 버려도 됨. 계속 들고 다니면 턴마다 훑는 양만 늘어남
- 원자 다 사라지면 break. 이거 없으면 4000턴 꽉 채워 돌린다
- 방향 인덱스에서 상(0)이 y 증가인 거 확인하고 dx/dy 짜자. 여기 틀리면 충돌이 아예 안 난다
