...
### 핵심 아이디어
빈 칸 전부 곱하기 4방향 전부 굴려보는 완전탐색. 반사 규칙은 블록 번호별로 들어온 방향 > 나가는 방향 매핑 테이블 만들어서 박아뒀다.

### 시간복잡도
O(T * N^2 * 4 * 경로 길이). N이 100이면 시작 경우가 4만개인데 블랙홀이나 시작점 복귀로 경로가 끊겨서 통과.

### 구현 포인트
- 외곽에 5번 블록(정사각형)으로 패딩 한 겹 둘렀다. 벽 반사가 5번 블록 반사랑 똑같아서 벽 처리 분기가 통째로 사라짐
- 웜홀 좌표는 미리 다 모아두고, 들어온 쪽이 a면 b로 아니면 a로 보냄. 방향은 그대로 유지
- i, j = ni, nj 위치 업데이트 빼먹어서 박았다. 웜홀 타고 ni, nj 바꿔놓고 반영을 안 하면 그대로 제자리
- 웜홀 통과는 점수 없다. 블록이나 벽에 박을 때만 +1
